...
3. Configure guided provider authorization:
   - Shopify: set `SHOPIFY_CLIENT_ID`, `SHOPIFY_CLIENT_SECRET`, and callback `${NEXT_PUBLIC_APP_URL}/api/performance/shopify/callback`. The default scopes are `read_orders,read_customers,read_products,read_reports`. ShopifyQL reporting also needs protected customer data Level 2 approval. Apply migration `055_performance_behavior_analytics.sql`, then add the generated consent-aware custom pixel from Performance integrations to collect all page views and product/cart/checkout events.
