...
export function PerformanceHQ({
  ...
  return <div className="performance-layout performance-hq">
    ...
    <div className="performance-heading-row"><PageHeader title={clientView ? `${account.displayName} Performance` : 'Performance HQ'} description={clientView ? 'Your live commerce, acquisition, and lifecycle performance.' : 'Store truth, paid attribution, lifecycle contribution, and the decisions behind them.'} />{!clientView ? <div className="performance-heading-actions"><PerformanceAccountPicker accounts={data.accounts.map(item => ({ id: item.id, displayName: item.displayName }))} value={account.id} /><PerformanceSyncControl accountId={account.id} sources={data.connections.map(connection => PERFORMANCE_PROVIDER_CATALOG.find(provider => provider.id === connection.provider)?.name || connection.provider)} /></div> : null}</div>
    ...
    <section className={`performance-hq-top${clientView ? ' is-client-view' : ''}`}><article className="performance-panel performance-trend-panel"><header className="performance-panel-head"><div><span className="performance-eyebrow">Daily trend · comparison, not attribution</span><h2>Store revenue vs ad spend</h2></div><span className="performance-panel-kpi">{formatPerformanceRatio(summary.roas)} MER</span></header><PerformanceChart rows={data.daily} currency={account.currency} /></article>{!clientView ? <ActionQueue data={data} /> : null}</section>
