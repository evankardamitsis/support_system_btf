...
function sampleRows(days: number, offset = 0) {
  ...
  for (let index = days - 1; index >= 0; index -= 1) {
    ...
    rows.push(
      row(date, 'shopify', { revenue, orders, conversions: orders, sessions: 1180 + index * 5, newCustomers: Math.round(orders * 0.62) }),
...
function sampleEntities(): PerformanceEntityMetric[] {
  ...
  return [
    ...
    { ...base, provider: 'shopify', entityType: 'landing_page', entityId: '/collections/bestsellers', entityName: '/collections/bestsellers', revenue: 15390, spend: 0, orders: 148, conversions: 148, impressions: 0, clicks: 0, sessions: 148 },
    { ...base, provider: 'shopify', entityType: 'landing_page', entityId: '/products/signature-set', entityName: '/products/signature-set', revenue: 9960, spend: 0, orders: 87, conversions: 87, impressions: 0, clicks: 0, sessions: 87 },
